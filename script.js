...
const DB = {
  ...
  set session(v) {
    v ? localStorage.setItem('tf_session', JSON.stringify(v))
...
async function doRegister() {
  ...
  if (users.some(u => u.email === mail)) return showErr('البريد مسجّل مسبقاً');   
  const u = {
    user_id: Date.now(),
    name, email: mail,
...
async function doLogin() {
  ...
  if (!u) return showErr('البريد أو كلمة المرور غير صحيحة');           
...
async function doReset() {
  ...
  if (!u) return showErr('لا يوجد حساب بهذا البريد');  
  if (!u.active) return showErr('هذا الحساب معطّل. راجعي المدير');

  u.password = await hashPassword(pass);                      
  DB.users = users;                                          
...
function addTask() {
  ...
  if (!txt) return toast('اكتبي عنوان المهمة');        
